...
import pandas as pd
import re
from nomes import handleNomes, get_apelidos, get_nomes
...
df['MaterialRelacionado'] = df.apply(lambda row: handleMaterial(row['MaterialRelacionado']), axis=1)
...
df.to_json('tratado.json', orient='records', force_ascii=False)